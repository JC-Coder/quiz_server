...
const baseClassName =
  'group w-full rounded-xl border px-4 py-3 text-left text-sm font-medium leading-6 transition focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-brand-500/10 sm:min-h-14 sm:px-5 sm:text-[15px]';

const variantClassMap = {
  neutral: 'border-slate-200 bg-white text-slate-700 hover:border-brand-300 hover:bg-brand-50/50',
  pending: 'border-brand-500 bg-brand-50/50 font-semibold text-brand-700 ring-2 ring-brand-500/10',
...
function OptionButton({ text, index, state, disabled, onClick, legacyMath = false }) {
    ...
    >
      <span className="flex items-center gap-3">
        ...
        <span className="flex-1">
          <MathText legacyMath={legacyMath}>{text}</MathText>
        </span>
